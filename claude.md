...
- **7-Stage Execution Flow** — Every task follows this cycle from start to resolution:
  ...
  4. **4_Formula — Specs & Approval**: Follow or update the specs in `specs.md`. Document reasoning in `llm_thinking_log.md`. Create and update architectural plans as the project evolves. Get approval (specs + designs reviewed) before implementing in `5_Symbols`. This is the mandatory gating stage. → **Formula Agent**
